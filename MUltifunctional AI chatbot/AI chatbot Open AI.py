...
openai.api_key = "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"
...
bot = telebot.TeleBot("6006227544:AAEeYQ-ya9LdCbqRMkgFLNM7SlKtQ3DqdVo")
...
def generate_gptaudio(message):
    try:
        ...
        prompt_array = message.text.split()[1:]
        if len(prompt_array) < 2:
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id, text="Please provide a message after the command '/gpta'")
            return
...
def tts_handler(message):
    try:
        ...
        tts.save("dev_groupno_20_cse.mp3")
        with open("dev_groupno_20_cse.mp3", "rb") as f:
            bot.send_audio(chat_id=message.chat.id, reply_to_message_id=message.message_id, audio=f)
        os.remove("dev_groupno_20_cse.mp3")
    except Exception as e:
        try:
            bot.send_message(chat_id=message.chat.id, reply_to_message_id=message.message_id, text="Reply to a message.")
            print(e)
